...
public class LoggingAspect {
    ...
    @Pointcut("execution(private * com.capgemini.warehouse.service..*(..))")
    public void privateMethodsInServiceLayer() {}

    @Before("serviceLayer() || privateMethodsInServiceLayer()")
    public void logBefore(JoinPoint joinPoint) {
        logger.info( "Executing method: " + joinPoint.getSignature().getName());
        logger.debug( "Arguments: " + Arrays.toString(joinPoint.getArgs()));
    }

    @AfterReturning(pointcut = "serviceLayer() || privateMethodsInServiceLayer()", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        logger.info( "Method executed successfully: " + joinPoint.getSignature().getName());
        logger.debug( "Result: " + result);
    }

    @AfterThrowing(pointcut = "serviceLayer() || privateMethodsInServiceLayer()", throwing = "error")
